...
class UniverseRecordIn(BaseModel):
    record_type: Literal["universe_seed", "epoch_snapshot", "system_authority", "planet_authority", "atmosphere_solve", "hydrosphere_solve", "region_resolve", "zoom_transition", "atomic_patch", "branch_diff"]
    seed: str = Field(min_length=1, max_length=160)
    timeline_tick: str = Field(min_length=1, max_length=120)
    scope_id: str = Field(min_length=1, max_length=160)
    parent_scope_id: str | None = Field(default=None, max_length=160)
    solver_version: str = Field(default="v1", min_length=1, max_length=80)
    payload: dict[str, Any] = Field(default_factory=dict)


class UniverseRecordOut(UniverseRecordIn):
    record_id: str
    created_at: str
