...
from arc.services.universe import create_universe_record, list_universe_records
...
@router.post("/api/universe/records")
def post_universe_record(item: UniverseRecordIn, role: str = Depends(role_operator)):
    out = create_universe_record(item, actor_role=role)
    log(role, "create_universe_record", out["record_id"], out)
    return out


@router.get("/api/universe/records")
def get_universe_records(seed: str | None = None, scope_id: str | None = None, limit: int = Query(default=DEFAULT_LIMIT, ge=1, le=MAX_LIMIT)):
    return {"records": list_universe_records(seed=seed, scope_id=scope_id, limit=_bounded(limit))}
